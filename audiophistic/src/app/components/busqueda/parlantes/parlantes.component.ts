...
@Component({
  ...
  styleUrls: ['../compartir.css', './parlantes.component.css']
})
export class ParlantesComponent implements OnInit {
  ...
  consultar_parlantes() {
    this.productos_service.consultar_productos_por_tipo(3).subscribe(
      (res: any) => {
        this.toastr.clear();
        if (res.body.error) {
  ...
  consultar_filtro_marcas() {
    this.busquedas_service.consultar_marcas_parlantes().subscribe(
      (res: any) => {
        this.toastr.clear();
        if (res.body.error) {
          this.toastr.error(res.body.error, 'Error', { timeOut: 5000 });
  ...
  consultar_filtro_conexiones() {
    this.busquedas_service.consultar_tipos_conexiones_parlantes().subscribe(
      (res: any) => {
        this.toastr.clear();
        if (res.body.error) {
          this.toastr.error(res.body.error, 'Error', { timeOut: 5000 });
  ...
  consultar_filtro_precios() {
    this.busquedas_service.consultar_precios_parlantes().subscribe(
      (res: any) => {
        this.toastr.clear();
        if (res.body.error) {
          this.toastr.error(res.body.error, 'Error', { timeOut: 5000 });
